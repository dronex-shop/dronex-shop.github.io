var last_buyer = {
	...
	ajax_get_last_clients : function(){
		...
		$.ajax({
			url: "../include/classes/Class.LastBuyer.php?lang="+$.cookie('PAGE_LANG'), 
			dataType: "json",
			type: 'POST',
			data: data,
	...
	change_client : function(clients){
		...
		if(client){
			...
			if($('.last-buyer-text').length){//all pages except intl
				...
				if(client['first_paid_time_hours_ago']>0){
					...
					if(client['first_paid_time_hours_ago']>day_interval){
						var days = Math.floor(client['first_paid_time_hours_ago']/day_interval);
